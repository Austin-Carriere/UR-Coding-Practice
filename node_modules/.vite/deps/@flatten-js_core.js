//#region node_modules/@flatten-js/core/dist/main.mjs
/**
...
var Errors = class {
	...
	*/
	static get ILLEGAL_PARAMETERS() {
		return /* @__PURE__ */ new ReferenceError("Illegal Parameters");
	}
	/**
	...
	*/
	static get ZERO_DIVISION() {
		return /* @__PURE__ */ new Error("Zero division");
	}
	/**
	...
	*/
	static get UNRESOLVED_BOUNDARY_CONFLICT() {
		return /* @__PURE__ */ new Error("Unresolved boundary conflict in boolean operation");
	}
	/**
	...
	*/
	static get INFINITE_LOOP() {
		return /* @__PURE__ */ new Error("Infinite loop");
	}
	static get CANNOT_COMPLETE_BOOLEAN_OPERATION() {
		return /* @__PURE__ */ new Error("Cannot complete boolean operation");
	}
	static get CANNOT_INVOKE_ABSTRACT_METHOD() {
		return /* @__PURE__ */ new Error("Abstract method cannot be invoked");
	}
	static get OPERATION_IS_NOT_SUPPORTED() {
		return /* @__PURE__ */ new Error("Operation is not supported");
	}
	static get UNSUPPORTED_SHAPE_TYPE() {
		return /* @__PURE__ */ new Error("Unsupported shape type");
	}
};
...
var LinkedList = class {
	...
	*/
	static testInfiniteLoop(first) {
		...
		do {
			if (edge != first && edge === controlEdge) throw Errors.INFINITE_LOOP;
			edge = edge.next;
			controlEdge = controlEdge.next.next;
		} while (edge != first);
	}
};
var defaultAttributes = { stroke: "black" };
var SVGAttributes = class {
	...
	toAttributesString() {
		return Object.keys(this).reduce((acc, key) => acc + (this[key] !== void 0 ? this.toAttrString(key, this[key]) : ""), ``);
	}
...
function intersectLine2Circle(line, circle) {
	...
	if (Flatten.Utils.EQ(dist, circle.r)) ip.push(prj);
...
function intersectLine2Box(line, box) {
	...
	for (let seg of box.toSegments()) {
		...
		for (let pt of ips_tmp) if (!ptInIntPoints(pt, ips)) ips.push(pt);
	}
	return ips;
}
function intersectLine2Arc(line, arc) {
	let ip = [];
	if (intersectLine2Box(line, arc.box).length === 0) return ip;
	let ip_tmp = intersectLine2Circle(line, new Flatten.Circle(arc.pc, arc.r));
...
function intersectSegment2Line(seg, line) {
	...
	return intersectLine2Line(new Flatten.Line(seg.ps, seg.pe), line);
}
function intersectSegment2Segment(seg1, seg2) {
	...
	if (line1.incidentTo(line2)) {
		...
		if (!Flatten.Utils.EQ_0(r_cross_s)) {
			...
			if (Flatten.Utils.GE(t, 0) && Flatten.Utils.LE(t, 1) && Flatten.Utils.GE(u, 0) && Flatten.Utils.LE(u, 1)) ip.push(snapToSegmentEndpoints(seg1.ps.translate(r.multiply(t)), seg1, seg2));
		}
	} else {
		let new_ip = intersectLine2Line(line1, line2);
		if (new_ip.length > 0) {
			if (isPointInSegmentBox(new_ip[0], seg1) && isPointInSegmentBox(new_ip[0], seg2)) ip.push(snapToSegmentEndpoints(new_ip[0], seg1, seg2));
		}
	}
	return ip;
}
function snapToSegmentEndpoints(pt, seg1, seg2) {
	for (const endpoint of [
		...
		seg2.ps,
		seg2.pe
	]) if (pt.equalTo(endpoint)) return endpoint;
	return pt;
}
...
function intersectSegment2Circle(segment, circle) {
	...
	if (segment.isZeroLength()) {
		...
		if (Flatten.Utils.EQ(dist, circle.r)) ips.push(segment.ps);
		return ips;
	}
	let ips_tmp = intersectLine2Circle(new Flatten.Line(segment.ps, segment.pe), circle);
...
function intersectSegment2Arc(segment, arc) {
	...
	let ip_tmp = intersectLine2Circle(new Flatten.Line(segment.ps, segment.pe), new Flatten.Circle(arc.pc, arc.r));
...
function intersectCircle2Circle(circle1, circle2) {
	...
	if (Flatten.Utils.GT(dist, r1 + r2)) return ip;
	if (Flatten.Utils.LT(dist, Math.abs(r1 - r2))) return ip;
	vec.x /= dist;
	vec.y /= dist;
	let pt;
...
function intersectArc2Circle(arc, circle) {
	...
	let ip_tmp = intersectCircle2Circle(circle, new Flatten.Circle(arc.pc, arc.r));
	for (let pt of ip_tmp) if (pt.on(arc)) ip.push(pt);
	return ip;
}
...
function intersectSegment2Polygon(segment, polygon) {
	let ip = [];
	for (let edge of polygon.edges) for (let pt of intersectEdge2Segment(edge, segment)) ip.push(pt);
	return ip;
}
function intersectArc2Polygon(arc, polygon) {
	let ip = [];
	for (let edge of polygon.edges) for (let pt of intersectEdge2Arc(edge, arc)) ip.push(pt);
	return ip;
}
function intersectLine2Polygon(line, polygon) {
	let ip = [];
	if (polygon.isEmpty()) return ip;
	for (let edge of polygon.edges) for (let pt of intersectEdge2Line(edge, line)) if (!ptInIntPoints(pt, ip)) ip.push(pt);
	return line.sortPoints(ip);
}
function intersectCircle2Polygon(circle, polygon) {
	let ip = [];
	if (polygon.isEmpty()) return ip;
	for (let edge of polygon.edges) for (let pt of intersectEdge2Circle(edge, circle)) ip.push(pt);
	return ip;
}
function intersectEdge2Edge(edge1, edge2) {
	if (edge1.isSegment) return intersectEdge2Segment(edge2, edge1.shape);
	else if (edge1.isArc) return intersectEdge2Arc(edge2, edge1.shape);
	else if (edge1.isLine) return intersectEdge2Line(edge2, edge1.shape);
	else if (edge1.isRay) return intersectEdge2Ray(edge2, edge1.shape);
	return [];
}
function intersectEdge2Polygon(edge, polygon) {
	...
	for (let resp_edge of resp_edges) ip = [...ip, ...intersectEdge2Edge(edge, resp_edge)];
	return ip;
}
function intersectMultiline2Polygon(multiline, polygon) {
	let ip = [];
	if (polygon.isEmpty() || multiline.size === 0) return ip;
	for (let edge of multiline) ip = [...ip, ...intersectEdge2Polygon(edge, polygon)];
	return ip;
}
function intersectPolygon2Polygon(polygon1, polygon2) {
	...
	for (let edge1 of polygon1.edges) ip = [...ip, ...intersectEdge2Polygon(edge1, polygon2)];
	return ip;
}
function intersectShape2Polygon(shape, polygon) {
	if (shape instanceof Flatten.Line) return intersectLine2Polygon(shape, polygon);
	else if (shape instanceof Flatten.Segment) return intersectSegment2Polygon(shape, polygon);
	else if (shape instanceof Flatten.Arc) return intersectArc2Polygon(shape, polygon);
	else return [];
}
...
function intersectShape2Multiline(shape, multiline) {
	let ip = [];
	for (let edge of multiline) ip = [...ip, ...intersectShape2Shape(shape, edge.shape)];
	return ip;
}
function intersectMultiline2Multiline(multiline1, multiline2) {
	let ip = [];
	for (let edge1 of multiline1) for (let edge2 of multiline2) ip = [...ip, ...intersectShape2Shape(edge1.shape, edge2.shape)];
	return ip;
}
/**
...
var Multiline$1 = class Multiline extends LinkedList {
	constructor(...args) {
		...
		if (args.length === 1 && args[0] instanceof Array && args[0].length > 0) {
			...
			if (L === 1 && anyShape(shapes[0]) || L > 1 && anyShapeExceptLine(shapes[0]) && anyShapeExceptLine(shapes[L - 1]) && shapes.slice(1, L - 1).every(shapeSegmentOrArc)) {
				...
				this.setArcLength();
			} else throw Flatten.Errors.ILLEGAL_PARAMETERS;
		}
	}
	/**
	...
	*/
	...
	setOneEdgeArcLength(edge) {
		if (edge === this.first) edge.arc_length = 0;
	...
	*/
	distanceTo(shape) {
		...
		if (shape instanceof Flatten.Multiline) return Flatten.Distance.multiline2multiline(this, shape);
	...
	*/
	intersect(shape) {
		if (shape instanceof Flatten.Multiline) return intersectMultiline2Multiline(this, shape);
		else return intersectShape2Multiline(shape, this);
	}
	/**
	...
	*/
	contains(shape) {
		if (shape instanceof Flatten.Point) return this.edges.some((edge) => edge.shape.contains(shape));
		throw Flatten.Errors.UNSUPPORTED_SHAPE_TYPE;
	}
	/**
...
function addToIntPoints(edge, pt, int_points) {
	...
	if (EQ(len, 0)) is_vertex |= START_VERTEX$1;
	if (EQ(len, edge.shape.length)) is_vertex |= END_VERTEX$1;
	let arc_length;
	if (len === Infinity) arc_length = shapes[0].coord(pt);
	else arc_length = is_vertex & END_VERTEX$1 && edge.next && edge.next.arc_length === 0 ? 0 : edge.arc_length + len;
	int_points.push({
		id,
		pt,
		arc_length,
...
function getSortedArray(int_points) {
	...
	return int_points.slice().sort(compareFn);
}
...
function filterDuplicatedIntersections(intersections) {
	...
	for (let i = 0; i < intersections.int_points1_sorted.length; i++) {
		...
		for (let j = i + 1; j < intersections.int_points1_sorted.length; j++) {
			...
			if (!EQ(int_point_cur1.arc_length, int_point_ref1.arc_length)) break;
...
function initializeInclusionFlags(int_points) {
	...
	for (let int_point of int_points) {
		if (int_point.edge_before) int_point.edge_before.bvEnd = 2;
		if (int_point.edge_after) int_point.edge_after.bvStart = 2;
	}
}
...
function setOverlappingFlags(intersections) {
	...
	for (let i = 0; i < num_int_points; i++) {
		...
		if (!(edge_from1.bv === 2 && edge_to1.bv === 2)) continue;
		...
		if (!(edge_from2.bv === 2 && edge_to2.bv === 2 && edge_from2 === edge_to2)) {
		...
		if (!(edge_from2.bv === 2 && edge_to2.bv === 2 && edge_from2 === edge_to2)) continue;
...
function intPointsPoolCount(int_points, cur_int_point_num, cur_face) {
	...
	for (let i = cur_int_point_num + 1; i < int_points.length; i++) {
		...
		if (!(int_point_next.pt.equalTo(int_point_current.pt) && int_point_next.edge_before === int_point_current.edge_before && int_point_next.edge_after === int_point_current.edge_after)) break;
		int_points_pool_num++;
	}
...
function splitByIntersections(polygon, int_points) {
	...
	for (let int_point of int_points) {
		...
		if (edge.shape.start && edge.shape.start.equalTo(int_point.pt)) int_point.is_vertex |= START_VERTEX$1;
		if (edge.shape.end && edge.shape.end.equalTo(int_point.pt)) int_point.is_vertex |= END_VERTEX$1;
		if (int_point.is_vertex & START_VERTEX$1) {
			int_point.edge_before = edge.prev;
			if (edge.prev) int_point.is_vertex = END_VERTEX$1;
			continue;
		}
		if (int_point.is_vertex & END_VERTEX$1) continue;
		int_point.edge_before = polygon.addVertex(int_point.pt, edge);
	}
	for (let int_point of int_points) if (int_point.edge_before) int_point.edge_after = int_point.edge_before.next;
	else if (polygon instanceof Multiline$1 && int_point.is_vertex & START_VERTEX$1) int_point.edge_after = polygon.first;
}
...
function subtract(polygon1, polygon2) {
	let [res_poly, wrk_poly] = booleanOpBinary(polygon1, polygon2.clone().reverse(), BOOLEAN_SUBTRACT, true);
	return res_poly;
}
/**
...
function innerClip(polygon1, polygon2) {
	...
	let clip_shapes1 = [];
	for (let face of res_poly.faces) clip_shapes1 = [...clip_shapes1, ...[...face.edges].map((edge) => edge.shape)];
	let clip_shapes2 = [];
	for (let face of wrk_poly.faces) clip_shapes2 = [...clip_shapes2, ...[...face.edges].map((edge) => edge.shape)];
...
function calculateIntersections(polygon1, polygon2) {
	...
	sortIntersections(intersections);
	return [intersections.int_points1_sorted.map((int_point) => int_point.pt), intersections.int_points2_sorted.map((int_point) => int_point.pt)];
}
...
function booleanOpBinary(polygon1, polygon2, op, restore) {
	...
	if (restore) swapLinksAndRestore(res_poly, wrk_poly, intersections, op);
		...
		if (edge_from1.bv === BOUNDARY && edge_to1.bv === BOUNDARY && edge_from1 != edge_to1) {
			...
			while (edge_tmp != edge_to1) {
				if (edge_tmp.bv != BOUNDARY) {
					if (new_bv === void 0) new_bv = edge_tmp.bv;
					else if (edge_tmp.bv != new_bv) throw Errors.UNRESOLVED_BOUNDARY_CONFLICT;
				}
				edge_tmp = edge_tmp.next;
			}
			if (new_bv != void 0) {
			...
			while (edge_tmp != edge_to1) {
				if (edge_tmp.bvStart === edge_from1.bv && edge_tmp.bvEnd === edge_to1.bv) {
					...
					if (dist < 10 * Flatten.DP_TOL) {
						...
						} else if (int_point2.is_vertex & END_VERTEX) int_point2.edge_after = edge2.next;
						else {
...
function removeNotRelevantChains(polygon, op, int_points, is_res_polygon) {
	...
	for (let i = 0; i < int_points.length; i++) {
		...
		if (edge_from.bv === INSIDE$1 && edge_to.bv === INSIDE$1 && op === BOOLEAN_UNION || edge_from.bv === OUTSIDE && edge_to.bv === OUTSIDE && op === BOOLEAN_INTERSECT || (edge_from.bv === OUTSIDE || edge_to.bv === OUTSIDE) && op === BOOLEAN_SUBTRACT && !is_res_polygon || (edge_from.bv === INSIDE$1 || edge_to.bv === INSIDE$1) && op === BOOLEAN_SUBTRACT && is_res_polygon || edge_from.bv === BOUNDARY && edge_to.bv === BOUNDARY && edge_from.overlap & OVERLAP_SAME && is_res_polygon || edge_from.bv === BOUNDARY && edge_to.bv === BOUNDARY && edge_from.overlap & OVERLAP_OPPOSITE) {
			...
			for (let k = int_points_from_pull_start; k < int_points_from_pull_start + int_points_from_pull_num; k++) int_points[k].edge_after = void 0;
			for (let k = int_points_to_pull_start; k < int_points_to_pull_start + int_points_to_pull_num; k++) int_points[k].edge_before = void 0;
		}
...
function copyWrkToRes(res_polygon, wrk_polygon, op, int_points) {
	for (let face of wrk_polygon.faces) {
		...
		if (int_points.find((ip) => ip.face === face) === void 0) res_polygon.addFace(face.first, face.last);
	}
}
function swapLinks(res_polygon, wrk_polygon, intersections) {
	...
	for (let i = 0; i < intersections.int_points1.length; i++) {
		...
		if (int_point1.edge_before !== void 0 && int_point1.edge_after === void 0) for (let int_point of intersections.int_points1_sorted) {
		...
		if (int_point2.edge_before !== void 0 && int_point2.edge_after === void 0) for (let int_point of intersections.int_points2_sorted) {
...
function removeOldFaces(polygon, int_points) {
	for (let int_point of int_points) {
		...
		if (int_point.edge_before) int_point.edge_before.face = void 0;
		if (int_point.edge_after) int_point.edge_after.face = void 0;
	}
}
function restoreFaces(polygon, int_points, other_int_points) {
	for (let int_point of int_points) {
		if (int_point.edge_before === void 0 || int_point.edge_after === void 0) continue;
...
function removeNotRelevantNotIntersectedFaces(polygon, notIntersectedFaces, op, is_res_polygon) {
	for (let face of notIntersectedFaces) {
		...
		if (op === BOOLEAN_UNION && rel === INSIDE$1 || op === BOOLEAN_SUBTRACT && rel === INSIDE$1 && is_res_polygon || op === BOOLEAN_SUBTRACT && rel === OUTSIDE && !is_res_polygon || op === BOOLEAN_INTERSECT && rel === OUTSIDE) polygon.deleteFace(face);
	}
}
function removeDetachedEdges(polygon) {
	const detachedEdges = [];
	for (const edge of polygon.edges) if (!edge.face || !polygon.faces.has(edge.face)) detachedEdges.push(edge);
	for (const edge of detachedEdges) polygon.edges.delete(edge);
}
...
function ray_shoot(polygon, point) {
	...
	if (polygon.box.not_intersect(searchBox)) return Flatten.OUTSIDE;
	let resp_edges = polygon.edges.search(searchBox);
	if (resp_edges.length === 0) return Flatten.OUTSIDE;
	for (let edge of resp_edges) if (edge.shape.contains(point)) return Flatten.BOUNDARY;
	let faces = [...polygon.faces];
	let intersections = [];
	for (let edge of resp_edges) for (let ip of ray.intersect(edge.shape)) {
		if (ip.equalTo(point)) return Flatten.BOUNDARY;
		intersections.push({
			pt: ip,
			edge,
	...
	intersections.sort((i1, i2) => {
		if (LT(i1.pt.x, i2.pt.x)) return -1;
		if (GT(i1.pt.x, i2.pt.x)) return 1;
	...
	for (let i = 0; i < intersections.length; i++) {
		...
		if (intersection.pt.equalTo(intersection.edge.shape.start)) {
			if (i > 0 && intersection.pt.equalTo(intersections[i - 1].pt) && intersection.face_index === intersections[i - 1].face_index && intersection.edge.prev === intersections[i - 1].edge) continue;
			let prev_edge = intersection.edge.prev;
			while (EQ_0(prev_edge.length)) prev_edge = prev_edge.prev;
			...
			if (i > 0 && intersection.pt.equalTo(intersections[i - 1].pt) && intersection.face_index === intersections[i - 1].face_index && intersection.edge.next === intersections[i - 1].edge) continue;
			let next_edge = intersection.edge.next;
			while (EQ_0(next_edge.length)) next_edge = next_edge.next;
		...
		} else if (intersection.edge.shape instanceof Flatten.Segment) counter++;
		else {
			let box = intersection.edge.shape.box;
			if (!(EQ(intersection.pt.y, box.ymin) || EQ(intersection.pt.y, box.ymax))) counter++;
		}
	}
	contains = counter % 2 === 1 ? 1 : 0;
	return contains;
}
/**
...
function relate(shape1, shape2) {
	if (shape1 instanceof Flatten.Line && shape2 instanceof Flatten.Line) return relateLine2Line(shape1, shape2);
	else if (shape1 instanceof Flatten.Line && shape2 instanceof Flatten.Circle) return relateLine2Circle(shape1, shape2);
	else if (shape1 instanceof Flatten.Line && shape2 instanceof Flatten.Box) return relateLine2Box(shape1, shape2);
	else if (shape1 instanceof Flatten.Line && shape2 instanceof Flatten.Polygon) return relateLine2Polygon(shape1, shape2);
	else if ((shape1 instanceof Flatten.Segment || shape1 instanceof Flatten.Arc) && shape2 instanceof Flatten.Polygon) return relateShape2Polygon(shape1, shape2);
	else if ((shape1 instanceof Flatten.Segment || shape1 instanceof Flatten.Arc) && (shape2 instanceof Flatten.Circle || shape2 instanceof Flatten.Box)) return relateShape2Polygon(shape1, new Flatten.Polygon(shape2));
	else if (shape1 instanceof Flatten.Polygon && shape2 instanceof Flatten.Polygon) return relatePolygon2Polygon(shape1, shape2);
	else if ((shape1 instanceof Flatten.Circle || shape1 instanceof Flatten.Box) && (shape2 instanceof Flatten.Circle || shape2 instanceof Flatten.Box)) return relatePolygon2Polygon(new Flatten.Polygon(shape1), new Flatten.Polygon(shape2));
	else if ((shape1 instanceof Flatten.Circle || shape1 instanceof Flatten.Box) && shape2 instanceof Flatten.Polygon) return relatePolygon2Polygon(new Flatten.Polygon(shape1), shape2);
	else if (shape1 instanceof Flatten.Polygon && (shape2 instanceof Flatten.Circle || shape2 instanceof Flatten.Box)) return relatePolygon2Polygon(shape1, new Flatten.Polygon(shape2));
}
function relateLine2Line(line1, line2) {
	let denim = new DE9IM();
	let ip = intersectLine2Line(line1, line2);
	if (ip.length === 0) if (line1.contains(line2.pt) && line2.contains(line1.pt)) {
		denim.I2I = [line1];
		denim.I2E = [];
		denim.E2I = [];
	} else {
...
function relateShape2Polygon(shape, polygon) {
	...
	denim.B2B = [];
	denim.B2E = [];
	for (let pt of [shape.start, shape.end]) switch (ray_shoot(polygon, pt)) {
		case Flatten.INSIDE:
			denim.B2I.push(pt);
			break;
	...
	*/
	translate(...args) {
		...
		if (args.length == 1 && !isNaN(args[0].x) && !isNaN(args[0].y)) {
			...
			tx = args[0];
			ty = args[1];
		} else throw Errors.ILLEGAL_PARAMETERS;
...
var Interval = class Interval extends IntervalBase {
	clone() {
...
var Node = class {
	constructor(key, value, left = null, right = null, parent = null, color = RB_TREE_COLOR_BLACK) {
		...
		if (value !== void 0) this.item.values.push(value);
		if (key !== void 0) if (Array.isArray(key)) {
	...
	update_max() {
		this.max = this.item.key ? this.item.key.max : void 0;
		if (this.right && this.right.max) this.max = this.max ? this.max.merge(this.right.max) : this.right.max;
		if (this.left && this.left.max) this.max = this.max ? this.max.merge(this.left.max) : this.left.max;
	}
...
*/
var IntervalTree = class IntervalTree {
	/**
	...
	*/
	get items() {
		...
		this.tree_walk(this.root, (node) => {
			...
			for (const v of node.item.values) res.push({
				key: keyOut,
				value: v
			});
		});
	...
	*/
	exist(key, value = key) {
		...
		if (arguments.length < 2 || value === key) return true;
	...
	*/
	remove(key, value = key) {
		...
		if (idx >= 0) {
			...
			if (node.item.values.length === 0) this.tree_delete(node);
			return node;
		}
	}
	search(interval, outputMapperFn = (value, key) => value === key ? key.output() : value) {
		...
		const res = [];
		for (const node of resp_nodes) for (const v of node.item.values) res.push(outputMapperFn(v, node.item.key));
		return res;
	}
	/**
	...
	*/
	map(callback) {
		...
		this.tree_walk(this.root, (node) => {
			for (const v of node.item.values) tree.insert(node.item.key, callback(v, node.item.key));
		});
		return tree;
	}
	*iterate(interval, outputMapperFn = (value, key) => value === key ? key.output() : value) {
		let node = null;
		if (interval) node = this.tree_search_nearest_forward(this.root, new Node(interval));
		else if (this.root) node = this.local_minimum(this.root);
		while (node) {
			for (const v of node.item.values) yield outputMapperFn(v, node.item.key);
	...
	*/
	insert_fixup(insert_node) {
		...
		while (current_node !== this.root && current_node.parent.color === RB_TREE_COLOR_RED) if (current_node.parent === current_node.parent.parent.left) {
	...
	*/
	tree_delete(delete_node) {
		...
		if (cut_node.color === RB_TREE_COLOR_BLACK) this.delete_fixup(fix_node);
	}
	/**
	...
	*/
	delete_fixup(fix_node) {
		...
		let brother_node;
		while (current_node !== this.root && current_node.parent != null && current_node.color === RB_TREE_COLOR_BLACK) if (current_node === current_node.parent.left) {
	...
	*/
	tree_search(node, search_node) {
		if (node == null || node === this.nil_node) return void 0;
	...
	*/
	tree_search_nearest_forward(node, search_node) {
		let best = null;
		let curr = node;
		while (curr && curr !== this.nil_node) if (curr.less_than(search_node)) if (curr.intersect(search_node)) {
			best = curr;
			curr = curr.left;
	...
	*/
	rotate_left(x) {
		const y = x.right;
		x.right = y.left;
		if (y.left !== this.nil_node) y.left.parent = x;
		y.parent = x.parent;
	...
	*/
	rotate_right(y) {
		const x = y.left;
		y.left = x.right;
		if (x.right !== this.nil_node) x.right.parent = y;
		x.parent = y.parent;
	...
	testRedBlackProperty() {
		...
		this.tree_walk(this.root, function(node) {
			if (node.color === RB_TREE_COLOR_RED) {
				if (!(node.left.color === RB_TREE_COLOR_BLACK && node.right.color === RB_TREE_COLOR_BLACK)) res = false;
			}
		});
		return res;
	}
	...
	*/
	testBlackHeightProperty(node) {
		...
		let heightLeft = 0;
		let heightRight = 0;
		if (node.color === RB_TREE_COLOR_BLACK) height++;
...
var PlanarSet = class extends Set {
	...
	*/
	svg() {
		return [...this].reduce((acc, shape) => acc + shape.svg(), "");
	}
};
...
* @type {Point}
*/
var Point$3 = class Point extends Shape {
	/**
	...
	*/
	projectionOn(line) {
		...
		if (Flatten.Utils.EQ_0(vec.cross(line.norm))) return line.pt.clone();
	...
	*/
	leftTo(line) {
		...
		return Flatten.Utils.GT(vec.dot(line.norm), 0);
	}
	/**
	...
	*/
	distanceTo(shape) {
		...
		if (shape instanceof Flatten.Line) return Flatten.Distance.point2line(this, shape);
		if (shape instanceof Flatten.Circle) return Flatten.Distance.point2circle(this, shape);
		if (shape instanceof Flatten.Segment) return Flatten.Distance.point2segment(this, shape);
		if (shape instanceof Flatten.Arc) return Flatten.Distance.point2arc(this, shape);
		if (shape instanceof Flatten.Polygon) return Flatten.Distance.point2polygon(this, shape);
		if (shape instanceof Flatten.PlanarSet) return Flatten.Distance.shape2planarSet(this, shape);
		if (shape instanceof Flatten.Multiline) return Flatten.Distance.shape2multiline(this, shape);
	}
	/**
	...
	*/
	on(shape) {
		if (shape instanceof Flatten.Point) return this.equalTo(shape);
...
var Vector$1 = class Vector extends Shape {
	...
	*/
	rotate(angle, center = new Flatten.Point()) {
		if (center.x === 0 && center.y === 0) return this.transform(new Matrix().rotate(angle));
...
var Segment$1 = class Segment extends Shape {
	...
	*/
	get slope() {
		return new Flatten.Vector(this.start, this.end).slope;
	}
	/**
	* Bounding box
	* @returns {Box}
	...
	*/
	intersect(shape) {
		if (shape instanceof Flatten.Point) return this.contains(shape) ? [shape] : [];
		if (shape instanceof Flatten.Line) return intersectSegment2Line(this, shape);
		if (shape instanceof Flatten.Ray) return intersectRay2Segment(shape, this);
		if (shape instanceof Flatten.Segment) return intersectSegment2Segment(this, shape);
		if (shape instanceof Flatten.Circle) return intersectSegment2Circle(this, shape);
		if (shape instanceof Flatten.Box) return intersectSegment2Box(this, shape);
		if (shape instanceof Flatten.Arc) return intersectSegment2Arc(this, shape);
		if (shape instanceof Flatten.Polygon) return intersectSegment2Polygon(this, shape);
		if (shape instanceof Flatten.Multiline) return intersectShape2Multiline(this, shape);
	}
	/**
	...
	*/
	tangentInStart() {
		return new Flatten.Vector(this.start, this.end).normalize();
	}
	/**
	...
	*/
	tangentInEnd() {
		return new Flatten.Vector(this.end, this.start).normalize();
	}
	/**
	...
	*/
	...
	definiteIntegral(ymin = 0) {
		return (this.end.x - this.start.x) * (this.start.y - ymin + (this.end.y - ymin)) / 2;
	}
	/**
	...
	*/
	sortPoints(pts) {
		return new Flatten.Line(this.start, this.end).sortPoints(pts);
	}
	get name() {
		return "segment";
	}
...
var Line$1 = class Line extends Shape {
		...
		*/
		...
		if (args.length === 2) {
			...
			if (a1 instanceof Flatten.Point && a2 instanceof Flatten.Point) {
				...
				if (this.norm.dot(vector(this.pt.x, this.pt.y)) >= 0) this.norm.invert();
				return;
			}
			if (a1 instanceof Flatten.Point && a2 instanceof Flatten.Vector) {
				if (Flatten.Utils.EQ_0(a2.x) && Flatten.Utils.EQ_0(a2.y)) throw Errors.ILLEGAL_PARAMETERS;
				this.pt = a1.clone();
				this.norm = a2.clone();
			...
			if (a1 instanceof Flatten.Vector && a2 instanceof Flatten.Point) {
				if (Flatten.Utils.EQ_0(a1.x) && Flatten.Utils.EQ_0(a1.y)) throw Errors.ILLEGAL_PARAMETERS;
				this.pt = a2.clone();
				this.norm = a1.clone();
				this.norm = this.norm.normalize();
				if (this.norm.dot(vector(this.pt.x, this.pt.y)) >= 0) this.norm.invert();
	...
	*/
	get slope() {
		return new Flatten.Vector(this.norm.y, -this.norm.x).slope;
	}
	/**
	...
	*/
	get standard() {
		return [
			this.norm.x,
			this.norm.y,
			this.norm.dot(vector(this.pt.x, this.pt.y))
		];
	}
	/**
	...
	*/
	intersect(shape) {
		if (shape instanceof Flatten.Point) return this.contains(shape) ? [shape] : [];
		if (shape instanceof Flatten.Line) return intersectLine2Line(this, shape);
		if (shape instanceof Flatten.Ray) return intersectRay2Line(shape, this);
		if (shape instanceof Flatten.Circle) return intersectLine2Circle(this, shape);
		if (shape instanceof Flatten.Box) return intersectLine2Box(this, shape);
		if (shape instanceof Flatten.Segment) return intersectSegment2Line(shape, this);
		if (shape instanceof Flatten.Arc) return intersectLine2Arc(this, shape);
		if (shape instanceof Flatten.Polygon) return intersectLine2Polygon(this, shape);
		if (shape instanceof Flatten.Multiline) return intersectShape2Multiline(this, shape);
	}
	/**
	...
	*/
	split(pt) {
		if (pt instanceof Flatten.Point) return [new Flatten.Ray(pt, this.norm), new Flatten.Ray(pt, this.norm)];
		else {
	...
	*/
	svg(box, attrs = {}) {
		...
		if (pe === void 0) pe = ps;
		return new Flatten.Segment(ps, pe).svg(attrs);
	}
	static points2norm(pt1, pt2) {
		if (pt1.equalTo(pt2)) throw Errors.ILLEGAL_PARAMETERS;
		return new Flatten.Vector(pt1, pt2).normalize().rotate90CCW();
	}
};
Flatten.Line = Line$1;
/**
...
var Circle$1 = class Circle extends Shape {
	...
	*/
	contains(shape) {
		if (shape instanceof Flatten.Point) return Flatten.Utils.LE(shape.distanceTo(this.center)[0], this.r);
		if (shape instanceof Flatten.Segment) return Flatten.Utils.LE(shape.start.distanceTo(this.center)[0], this.r) && Flatten.Utils.LE(shape.end.distanceTo(this.center)[0], this.r);
		if (shape instanceof Flatten.Arc) return this.intersect(shape).length === 0 && Flatten.Utils.LE(shape.start.distanceTo(this.center)[0], this.r) && Flatten.Utils.LE(shape.end.distanceTo(this.center)[0], this.r);
		if (shape instanceof Flatten.Circle) return this.intersect(shape).length === 0 && Flatten.Utils.LE(shape.r, this.r) && Flatten.Utils.LE(shape.center.distanceTo(this.center)[0], this.r);
	}
	/**
	...
	*/
	scale(sx, sy) {
		...
		if (!(this.pc.x === 0 && this.pc.y === 0)) throw Errors.OPERATION_IS_NOT_SUPPORTED;
	...
	*/
	intersect(shape) {
		if (shape instanceof Flatten.Point) return this.contains(shape) ? [shape] : [];
		if (shape instanceof Flatten.Line) return intersectLine2Circle(shape, this);
		if (shape instanceof Flatten.Ray) return intersectRay2Circle(shape, this);
		if (shape instanceof Flatten.Segment) return intersectSegment2Circle(shape, this);
		if (shape instanceof Flatten.Circle) return intersectCircle2Circle(shape, this);
		if (shape instanceof Flatten.Box) return intersectCircle2Box(this, shape);
		if (shape instanceof Flatten.Arc) return intersectArc2Circle(shape, this);
		if (shape instanceof Flatten.Polygon) return intersectCircle2Polygon(this, shape);
		if (shape instanceof Flatten.Multiline) return intersectShape2Multiline(this, shape);
	}
	/**
...
var Arc = class extends Shape {
	...
	*/
	get sweep() {
		...
		if (Flatten.Utils.EQ(Math.abs(startAngle - endAngle), Flatten.PIx2)) return Flatten.PIx2;
		if (Math.abs(startAngle) > Flatten.PIx2) startAngle -= Math.trunc(startAngle / Flatten.PIx2) * Flatten.PIx2;
		if (startAngle < 0) startAngle += Flatten.PIx2;
		if (Math.abs(endAngle) > Flatten.PIx2) endAngle -= Math.trunc(endAngle / Flatten.PIx2) * Flatten.PIx2;
		if (endAngle < 0) endAngle += Flatten.PIx2;
		let sweep = this.counterClockwise ? endAngle - startAngle : startAngle - endAngle;
		if (sweep < 0) sweep += Flatten.PIx2;
		return sweep;
	}
	/**
	...
	*/
	get start() {
		return new Flatten.Point(this.pc.x + this.r, this.pc.y).rotate(this.startAngle, this.pc);
	}
	/**
	...
	*/
	get end() {
		return new Flatten.Point(this.pc.x + this.r, this.pc.y).rotate(this.endAngle, this.pc);
	}
	/**
	* Get center of arc
	* @returns {Point}
	...
	*/
	get box() {
		let box = this.breakToFunctional().reduce((acc, arc) => acc.merge(arc.start.box), new Flatten.Box());
	...
	*/
	middle() {
		...
		return new Flatten.Arc(this.pc, this.r, this.startAngle, endAngle, this.counterClockwise).end;
	}
	/**
	* Get point at given length
	...
	*/
	intersect(shape) {
		if (shape instanceof Flatten.Point) return this.contains(shape) ? [shape] : [];
		if (shape instanceof Flatten.Line) return intersectLine2Arc(shape, this);
		if (shape instanceof Flatten.Ray) return intersectRay2Arc(shape, this);
		if (shape instanceof Flatten.Circle) return intersectArc2Circle(this, shape);
		if (shape instanceof Flatten.Segment) return intersectSegment2Arc(shape, this);
		if (shape instanceof Flatten.Box) return intersectArc2Box(this, shape);
		if (shape instanceof Flatten.Arc) return intersectArc2Arc(this, shape);
		if (shape instanceof Flatten.Polygon) return intersectArc2Polygon(this, shape);
		if (shape instanceof Flatten.Multiline) return intersectShape2Multiline(this, shape);
	}
	/**
	...
	*/
	breakToFunctional() {
		...
		let startAngle = this.startAngle;
		let endAngle = this.endAngle;
		if (Flatten.Utils.EQ(Math.abs(startAngle - endAngle), Flatten.PIx2)) endAngle = startAngle;
		if (Math.abs(startAngle) > Flatten.PIx2) startAngle -= Math.trunc(startAngle / Flatten.PIx2) * Flatten.PIx2;
		if (startAngle < 0) startAngle += Flatten.PIx2;
		if (Math.abs(endAngle) > Flatten.PIx2) endAngle -= Math.trunc(endAngle / Flatten.PIx2) * Flatten.PIx2;
		if (endAngle < 0) endAngle += Flatten.PIx2;
		...
		for (let i = 0, j = firstj; i < 4; i++, j = (j + d + 4) % 4) {
			...
			if (incrementalSweep < 0) incrementalSweep += Flatten.PIx2;
			if (incrementalSweep > this.sweep) break;
		...
		next = endAngle;
		if (prev !== next) func_arcs_array.push(new Flatten.Arc(this.pc, this.r, prev, next, this.counterClockwise));
		return func_arcs_array;
	}
	/**
	...
	*/
	...
	definiteIntegral(ymin = 0) {
		return this.breakToFunctional().reduce((acc, arc) => acc + arc.circularSegmentDefiniteIntegral(ymin), 0);
	}
	circularSegmentDefiniteIntegral(ymin) {
		let areaTrapez = new Flatten.Segment(this.start, this.end).definiteIntegral(ymin);
	...
	circularSegmentArea() {
		return .5 * this.r * this.r * (this.sweep - Math.sin(this.sweep));
	}
	/**
	...
	*/
	svg(attrs = {}) {
		...
		if (Flatten.Utils.EQ(this.sweep, 2 * Math.PI)) return new Flatten.Circle(this.pc, this.r).svg(attrs);
...
var Box = class Box extends Shape {
	...
	*/
	transform(m = new Flatten.Matrix()) {
		return this.toPoints().map((pt) => pt.transform(m)).reduce((new_box, pt) => new_box.merge(pt.box), new Box());
	}
	/**
	...
	*/
	contains(shape) {
		if (shape instanceof Flatten.Point) return shape.x >= this.xmin && shape.x <= this.xmax && shape.y >= this.ymin && shape.y <= this.ymax;
		if (shape instanceof Flatten.Segment) return shape.vertices.every((vertex) => this.contains(vertex));
		if (shape instanceof Flatten.Box) return shape.toSegments().every((segment) => this.contains(segment));
		if (shape instanceof Flatten.Circle) return this.contains(shape.box);
		if (shape instanceof Flatten.Arc) return shape.vertices.every((vertex) => this.contains(vertex)) && this.toSegments().every((segment) => intersectSegment2Arc(segment, shape).length === 0);
		if (shape instanceof Flatten.Line || shape instanceof Flatten.Ray) return false;
		if (shape instanceof Flatten.Multiline) return shape.toShapes().every((shape) => this.contains(shape));
		if (shape instanceof Flatten.Polygon) return this.contains(shape.box);
	}
	/**
	...
	*/
	distanceTo(shape) {
		...
		let shortestDistanceInfo = [Number.MAX_SAFE_INTEGER, null];
...
var Edge = class {
	...
	*/
	setInclusion(polygon) {
		...
		if (this.bvStart === void 0) this.bvStart = ray_shoot(polygon, this.start);
		if (this.bvEnd === void 0) this.bvEnd = ray_shoot(polygon, this.end);
		if (this.bvStart === Flatten.OUTSIDE || this.bvEnd == Flatten.OUTSIDE) this.bv = Flatten.OUTSIDE;
		else if (this.bvStart === Flatten.INSIDE || this.bvEnd == Flatten.INSIDE) this.bv = Flatten.INSIDE;
		else {
	...
	*/
	setOverlap(edge) {
		...
		if (shape1 instanceof Flatten.Segment && shape2 instanceof Flatten.Segment) {
			if (shape1.start.equalTo(shape2.start) && shape1.end.equalTo(shape2.end)) flag = Flatten.OVERLAP_SAME;
			else if (shape1.start.equalTo(shape2.end) && shape1.end.equalTo(shape2.start)) flag = Flatten.OVERLAP_OPPOSITE;
		} else if (shape1 instanceof Flatten.Arc && shape2 instanceof Flatten.Arc) {
			if (shape1.start.equalTo(shape2.start) && shape1.end.equalTo(shape2.end) && shape1.middle().equalTo(shape2.middle())) flag = Flatten.OVERLAP_SAME;
			else if (shape1.start.equalTo(shape2.end) && shape1.end.equalTo(shape2.start) && shape1.middle().equalTo(shape2.middle())) flag = Flatten.OVERLAP_OPPOSITE;
	...
	svg() {
		if (this.shape instanceof Flatten.Segment) return ` L${this.shape.end.x},${this.shape.end.y}`;
...
*/
var Face = class Face extends CircularLinkedList {
		...
		if (args.length === 1) {
			if (args[0] instanceof Array) {
				...
				if (shapes.every((shape) => {
					...
					for (let shape of shapes) {
						let flattenShape;
						if (shape.name === "segment") flattenShape = new Flatten.Segment(shape);
						else flattenShape = new Flatten.Arc(shape);
			...
			} else if (args[0] instanceof Flatten.Circle) this.shapes2face(polygon.edges, [args[0].toArc(true)]);
	...
	*/
	orientation() {
		if (this._orientation === void 0) {
			...
			if (Flatten.Utils.EQ_0(area)) this._orientation = ORIENTATION.NOT_ORIENTABLE;
			else if (Flatten.Utils.LT(area, 0)) this._orientation = ORIENTATION.CCW;
			else this._orientation = ORIENTATION.CW;
		}
...
* @type {Ray}
*/
var Ray = class Ray extends Shape {
	/**
	...
	*/
	constructor(...args) {
		...
		this.norm = new Flatten.Vector(0, 1);
		if (args.length === 0) return;
		if (args.length >= 1 && args[0] instanceof Flatten.Point) this.pt = args[0].clone();
	...
	*/
	split(pt) {
		...
		if (this.pt.equalTo(pt)) return [this];
	...
	*/
	intersect(shape) {
		if (shape instanceof Flatten.Point) return this.contains(shape) ? [shape] : [];
		if (shape instanceof Flatten.Segment) return intersectRay2Segment(this, shape);
		if (shape instanceof Flatten.Arc) return intersectRay2Arc(this, shape);
		if (shape instanceof Flatten.Line) return intersectRay2Line(this, shape);
		if (shape instanceof Flatten.Ray) return intersectRay2Ray(this, shape);
		if (shape instanceof Flatten.Circle) return intersectRay2Circle(this, shape);
		if (shape instanceof Flatten.Box) return intersectRay2Box(this, shape);
		if (shape instanceof Flatten.Polygon) return intersectRay2Polygon(this, shape);
		if (shape instanceof Flatten.Multiline) return intersectShape2Multiline(this, shape);
	}
	/**
	* Return new line rotated by angle
	...
	*/
	svg(box, attrs = {}) {
		let ip = intersectLine2Box(new Flatten.Line(this.pt, this.norm), box);
		ip = ip.filter((pt) => this.contains(pt));
		if (ip.length === 0 || ip.length === 2) return "";
		return new Flatten.Segment(this.pt, ip[0]).svg(attrs);
	}
};
Flatten.Ray = Ray;
...
var Polygon$1 = class Polygon {
	...
	constructor() {
		...
		*/
		...
		if (args.length === 1 && (args[0] instanceof Array && args[0].length > 0 || args[0] instanceof Flatten.Circle || args[0] instanceof Flatten.Box)) {
			...
			else for (let loop of argsArray) if (loop instanceof Array && loop[0] instanceof Array && loop[0].every((el) => {
				return el instanceof Array && el.length === 2 && typeof el[0] === "number" && typeof el[1] === "number";
			})) for (let loop1 of loop) this.faces.add(new Flatten.Face(this, loop1));
			else this.faces.add(new Flatten.Face(this, loop));
			else this.faces.add(new Flatten.Face(this, argsArray));
		}
	}
	/**
	...
	*/
	cut(multiline) {
		const result = this.splitToIslands().flatMap((polygon) => polygon._cutSingleIsland(multiline)).filter((polygon) => polygon.isValid() && polygon.isEmpty() === false);
	...
	*/
	_cutSingleIsland(inputMultiline) {
		...
		for (let i = 1; i < intersections.int_points1_sorted.length; i++) {
			...
			if (int_point1_curr.edge_before && int_point1_curr.edge_before.bv === 1) {
	...
	*/
	splitToIslands() {
		...
		for (let polygon of polygons) {
			...
			for (let islandPolygon of newPolygons) if (face.shapes.every((shape) => islandPolygon.contains(shape))) {
	...
	distanceTo(shape) {
		...
		if (shape instanceof Flatten.Polygon) {
			...
			for (let edge of this.edges) {
				...
				if (Flatten.Utils.LT(dist, min_stop)) min_dist_and_segment = [dist, shortest_segment];
			}
	...
	*/
	intersect(shape) {
		if (shape instanceof Flatten.Point) return this.contains(shape) ? [shape] : [];
		if (shape instanceof Flatten.Line) return intersectLine2Polygon(shape, this);
		if (shape instanceof Flatten.Ray) return intersectRay2Polygon(shape, this);
		if (shape instanceof Flatten.Circle) return intersectCircle2Polygon(shape, this);
		if (shape instanceof Flatten.Segment) return intersectSegment2Polygon(shape, this);
		if (shape instanceof Flatten.Arc) return intersectArc2Polygon(shape, this);
		if (shape instanceof Flatten.Polygon) return intersectPolygon2Polygon(shape, this);
		if (shape instanceof Flatten.Multiline) return intersectMultiline2Polygon(shape, this);
	}
	/**
...
var { Circle, Line, Point: Point$2, Vector, Utils } = Flatten;
/**
...
var Inversion = class Inversion {
	...
	*/
	...
	static inversePoint(inversion_circle, point) {
		...
		const len2 = v.dot(v);
		return Utils.EQ_0(len2) ? new Point$2(Number.POSITIVE_INFINITY, Number.POSITIVE_INFINITY) : inversion_circle.pc.translate(v.multiply(k2 / len2));
	}
	static inverseCircle(inversion_circle, circle) {
		...
		if (Utils.EQ(dist, circle.r)) {
			...
			v = v.normalize();
			return new Line(inversion_circle.pc.translate(v.multiply(d)), v);
		} else {
			let v = new Vector(inversion_circle.pc, circle.pc);
			let s = inversion_circle.r * inversion_circle.r / (v.dot(v) - circle.r * circle.r);
			return new Circle(inversion_circle.pc.translate(v.multiply(s)), Math.abs(s) * circle.r);
		}
	}
	static inverseLine(inversion_circle, line) {
		const [dist, shortest_segment] = inversion_circle.pc.distanceTo(line);
		if (Utils.EQ_0(dist)) return line.clone();
		else {
	...
	inverse(shape) {
		if (shape instanceof Point$2) return Inversion.inversePoint(this.circle, shape);
		else if (shape instanceof Circle) return Inversion.inverseCircle(this.circle, shape);
		else if (shape instanceof Line) return Inversion.inverseLine(this.circle, shape);
	}
};
...
var Distance = class Distance {
	...
	*/
	static point2line(pt, line) {
		...
		return [new Flatten.Vector(pt, closest_point).length, new Flatten.Segment(pt, closest_point)];
	}
	/**
	...
	*/
	static point2circle(pt, circle) {
		...
		if (Flatten.Utils.EQ_0(dist2center)) return [circle.r, new Flatten.Segment(pt, circle.toArc().start)];
		else {
	...
	*/
	static point2segment(pt, segment) {
		if (segment.start.equalTo(segment.end)) return Distance.point2point(pt, segment.start);
	...
	*/
	static point2arc(pt, arc) {
		...
		if (shortest_segment.end.on(arc)) dist_and_segment.push(Distance.point2circle(pt, circle));
	...
	*/
	static segment2line(seg, line) {
		...
		if (ip.length > 0) return [0, new Flatten.Segment(ip[0], ip[0])];
		let dist_and_segment = [];
	...
	*/
	static segment2circle(seg, circle) {
		...
		if (Flatten.Utils.GE(dist, circle.r) && shortest_segment.end.on(seg)) return Distance.point2circle(shortest_segment.end, circle);
		else {
	...
	*/
	static segment2arc(seg, arc) {
		...
		if (Flatten.Utils.GE(dist_from_center, circle.r) && shortest_segment_from_center.end.on(seg)) {
			...
			if (shortest_segment_from_projection.end.on(arc)) return [dist_from_projection, shortest_segment_from_projection];
		}
		let dist_and_segment = [];
	...
	*/
	static arc2circle(arc, circle2) {
		...
		if (shortest_segment.start.on(arc)) return [dist, shortest_segment];
		else {
			let dist_and_segment = [];
	...
	*/
	static arc2arc(arc1, arc2) {
		...
		let [dist, shortest_segment] = Distance.circle2circle(circle1, circle2);
		if (shortest_segment.start.on(arc1) && shortest_segment.end.on(arc2)) return [dist, shortest_segment];
		else {
			let dist_and_segment = [];
			let dist_tmp, segment_tmp;
			[dist_tmp, segment_tmp] = Distance.point2arc(arc1.start, arc2);
			if (segment_tmp.end.on(arc2)) dist_and_segment.push([dist_tmp, segment_tmp]);
			[dist_tmp, segment_tmp] = Distance.point2arc(arc1.end, arc2);
			if (segment_tmp.end.on(arc2)) dist_and_segment.push([dist_tmp, segment_tmp]);
			[dist_tmp, segment_tmp] = Distance.point2arc(arc2.start, arc1);
			if (segment_tmp.end.on(arc1)) dist_and_segment.push([dist_tmp, segment_tmp.reverse()]);
	...
	*/
	static point2polygon(point, polygon) {
		...
		for (let edge of polygon.edges) {
			...
			if (Flatten.Utils.LT(dist, min_dist_and_segment[0])) min_dist_and_segment = [dist, shortest_segment];
		}
		return min_dist_and_segment;
	}
	...
	*/
	...
	static minmax_tree_process_level(shape, level, min_stop, tree) {
		...
		for (let node of level) {
			...
			for (let value of node.item.values) if (value instanceof Flatten.Edge) tree.insert([mindist, maxdist], value.shape);
			else tree.insert([mindist, maxdist], value);
			if (Flatten.Utils.LT(maxdist, min_stop)) min_stop = maxdist;
		}
	...
	*/
	...
	static minmax_tree_calc_distance(shape, node, min_dist_and_segment) {
		...
		if (node != null && !node.isNil()) {
			...
			if (stop) return [min_dist_and_segment_new, stop];
			if (Flatten.Utils.LT(min_dist_and_segment_new[0], Math.sqrt(node.item.key.low))) return [min_dist_and_segment_new, true];
			let [dist, shortest_segment] = Distance.distanceToArray(shape, node.item.values);
			if (Flatten.Utils.LT(dist, min_dist_and_segment_new[0])) min_dist_and_segment_new = [dist, shortest_segment];
	...
	*/
	...
	static sort(dist_and_segment) {
		dist_and_segment.sort((d1, d2) => {
			if (Flatten.Utils.LT(d1[0], d2[0])) return -1;
			if (Flatten.Utils.GT(d1[0], d2[0])) return 1;
			return 0;
		});
	}
...
var { Multiline, Point: Point$1, Segment, Polygon } = Flatten;
...
function parseMultiLineString(multilineStr) {
	return multilineStr.replace(/\(\(/, "").replace(/\)\)$/, "").split("), (").map(parseLineString);
}
...
function parseMutliPolygon(multiPolygonString) {
	const polygons = multiPolygonString.split(/\)\), \(\(/).map((polygon) => "((" + polygon + "))").map(parseSinglePolygon);
	const polygon = new Polygon();
	polygons.reduce((acc, polygon) => [...acc, ...polygon?.faces], []).forEach((face) => polygon.addFace([...face?.shapes]));
	return polygon;
}
function parsePolygon(wkt) {
	if (wkt.startsWith("POLYGON")) return parseSinglePolygon(wkt.replace(/^POLYGON /, ""));
	else return parseMutliPolygon(wkt.replace(/^MULTIPOLYGON \(\(\((.*)\)\)\)$/, "$1"));
}
function parseArrayOfPoints(str) {
	return str.split("\n").map((x) => x.match(/\(([^)]+)\)/)[1]).map(parseSinglePoint);
}
function parseArrayOfLineStrings(str) {
	return str.split("\n").map((x) => x.match(/\(([^)]+)\)/)[1]).map(parseLineString).reduce((acc, x) => [...acc, ...x], []);
}
/**
...
function parseWKT(str) {
	if (str.startsWith("POINT")) return parseSinglePoint(str.replace(/^POINT \(/, "").replace(/\)$/, ""));
	else if (str.startsWith("MULTIPOINT")) return parseMultiPoint(str.replace(/^MULTIPOINT \(/, "").replace(/\)$/, ""));
	else if (str.startsWith("LINESTRING")) return parseLineString(str.replace(/^LINESTRING \(/, "").replace(/\)$/, ""));
	else if (str.startsWith("MULTILINESTRING")) return parseMultiLineString(str.replace(/^MULTILINESTRING /, ""));
	else if (str.startsWith("POLYGON") || str.startsWith("MULTIPOLYGON")) return parsePolygon(str);
	else if (str.startsWith("GEOMETRYCOLLECTION")) {
		const wktArray = str.match(/(?<type>POINT|LINESTRING|POLYGON|MULTIPOINT|MULTILINESTRING|MULTIPOLYGON) \((?:[^\(\)]|\([^\)]*\))*\)/g);
		if (wktArray[0].startsWith("GEOMETRYCOLLECTION")) wktArray[0] = wktArray[0].replace("GEOMETRYCOLLECTION (", "");
		return wktArray.map(parseWKT).map((x) => x instanceof Array ? x : [x]).reduce((acc, x) => [...acc, ...x], []);
	} else if (isArrayOfPoints(str)) return parseArrayOfPoints(str);
	else if (isArrayOfLines(str)) return parseArrayOfLineStrings(str);
	return [];
}
